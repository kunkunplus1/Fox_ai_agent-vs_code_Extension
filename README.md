...
1. 获取扩展包 `fox-ai-1.1.30.vsix`（由源码经 `vsce package` 打包生成，或自发布渠道取得；实际文件名以你下载的版本为准）。
2. 打开扩展视图（侧边栏方块图标，或 `Ctrl+Shift+X`），点击右上角 `…` 选择「从 VSIX 安装」。
3. 选中 `fox-ai-1.1.30.vsix`，安装完成后按提示重新加载（Reload）窗口。
...
- **反馈带行号锚点**：写类工具的返回直接给出精确行信息，模型无需心算——整体覆盖显示「原 N 行 → 现 M 行」与真实增减（`-旧 +新`，不再出现只加不删的不对称统计）；编辑/删除附上带 1 索引行号的改动片段（`@@ -起,行数 +起,行数 @@` 标注变更范围），写完立刻知道改动落在第几行。
...
- **精准 diff 引擎**：差异预览基于行级对齐算法（只改动几行就只标几行，不再「把中间整段显示成先全删再全加」）；多处分散改动自动切分为多个 `@@` 变更块（hunk）；统计区分新增 / 删除 / 修改行（如 `+2 -1，其中 1 行修改`）。
- **行内词级高亮**：审批 / 预览面板中的修改行会标出词级差异——删除片段带删除线、新增片段高亮（如把 `a + b` 改成 `a * b`，只标出 `+`→`*`），一眼看清到底改了哪个符号；纯文本自检摘要保持原始行内容，不打断子串匹配。
- **对比维度可调**：diff 比较可忽略空白 / 大小写 / 空行差异（如改了个缩进或大小写不算内容变化），上下文行数与输出长度均可配置。
...
- **Webhook 安全**：监听默认仅绑定 `127.0.0.1`（不暴露到局域网）；必须配置 `foxAi.automations.webhookSecret`（至少 16 位随机字符串），未配置或强度不足时 webhook 拒绝启动——防止任意网页（CSRF）或同网段主机触发自动化任务。请求只校验任务 id + secret，白名单外的 id 一律拒绝。
