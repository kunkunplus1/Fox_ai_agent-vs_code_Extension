...
> **🦊 1.1.12 更新**：本版在 1.1.9 基础上，重点加强了「工具结果」与「各厂商模型」之间的适配一致性，进一步缩小不同模型产出质量的差距。
>
> - **工具结果智能截断（头 + 尾）**：命令输出、测试结果、日志等超长工具结果，从「只截头部」改为「保留头 60% + 尾 40%」——关键报错与失败摘要常在尾部，旧逻辑会把它丢掉，导致模型（尤其弱模型）误判“成功”或反复重试。现在与 MCP 工具结果的智能截断保持一致，统一了本地工具与远程工具的反馈形态。
> - **厂商专属适配（缩小原生 vs 第三方差距）**：为 DeepSeek / OpenAI / Claude 各注入一小段「专属工作风格与格式约定」到 system 提示词（随前缀缓存、字节稳定）——DeepSeek 强化 `<foxtool>` 文本协议格式与「先读后改、小步精确」；OpenAI 强调原生 function calling 与结构化输出；Claude 强调先暴露假设再动手、避免过度询问。可按 provider 自动选择，也可用 `foxAi.agent.providerProfile` 覆盖（`auto` / `deepseek` / `openai` / `claude` / `none` / 自定义文本）。
> - **厂商速度调优**：按各家延迟与输出特性套用默认超时与输出上限——DeepSeek `45s / 2048`、OpenAI `60s / 2048`、Claude `120s / 4096`（慢厂商给足超时、快厂商快速失败），本地模型 `120s / 1536`。用户显式设置 `foxAi.timeout` / `foxAi.maxTokens` 时优先于厂商默认。
>
> 以下为 1.1.9 已含内容，一并列出便于回顾：
>
...
1. 获取扩展包 `fox-ai-1.1.12.vsix`（由源码经 `vsce package` 打包生成，或自发布渠道取得；实际文件名以你下载的版本为准）。
2. 在 Visual Studio Code 中打开扩展视图（侧边栏方块图标，或 `Ctrl+Shift+X`）。
3. 点击扩展视图右上角的 `…`（更多操作），选择 **“从 VSIX 安装”**。
4. 在文件选择对话框中定位并选中 `fox-ai-1.1.12.vsix`。
...
配套细节：关闭时对默认就思考的模型会主动下发关闭参数；非流式自动规避思考参数；参数不被接受时自动去掉重试；思考过程仍以可折叠卡片展示。
...
模型回答里引用的资料会以可点击角标呈现，点击用系统浏览器打开来源。兼容中文来源标签、markdown 脚注 `[^n]`、`[n]` 编号索引；正文裸链接自动可点；多厂商原生联网搜索的真实 URL 统一汇入角标。
